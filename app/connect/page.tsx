...
function linkTitle(mode: string, fallback: string) {
  if (mode === "black_vless") return "⬛ GLOBAL MODE"
  if (mode === "white_mobile") return "⬜ STEADY MODE"
  return fallback
}
...
export default function ConnectPage() {
  ...
  useEffect(() => {
    ...
    if (!initData) {
      setLoading(false)
      setError("Откройте этот экран внутри Telegram через кнопку бота.")
      return
    }
            ...
            <section className="rounded-3xl border border-white/10 bg-white/[0.04] p-5">
              ...
              <div className="mt-4 space-y-3">
                ...
                <button
                  ...
                  className="w-full rounded-2xl bg-slate-700 px-4 py-4 text-left font-bold shadow-lg disabled:cursor-not-allowed disabled:opacity-55"
                >
                  ⬜ STEADY MODE
                  <span className="block pt-1 text-sm font-normal text-white/75">
